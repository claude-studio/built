...
3. **plan-draft.md 확인**: `.built/runs/<FEATURE>/plan-draft.md`가 존재하면 이전 세션의 중간 저장 파일이 있다는 의미다. AskUserQuestion으로 묻는다:
   ...
   - **"처음부터 다시" 선택 시**: `node -e "require('./scripts/plan-draft.js').remove('<FEATURE>')"` 를 실행해 draft를 삭제하고 Phase 0부터 시작한다.
...
Phase 1의 모든 응답 수집 후 다음 코드를 실행해 중간 저장한다:

```javascript
node -e "
const d = require('./scripts/plan-draft.js');
...
4. 수정이 완료되거나 "모두 승인" 전 draft를 최종 갱신한다:
...
프로젝트 루트에서 `node src/update-index.js`를 실행한다.

### 5-5. plan-draft.md 삭제

모든 저장이 완료된 후 중간 저장 파일을 삭제한다:

```javascript
node -e "require('./scripts/plan-draft.js').remove('<FEATURE>')"
```

...
# /built:run-sonnet — Claude provider claude-sonnet-4-5 모델 preset 실행

feature spec을 읽어 Do→Check→Iter→Report 파이프라인을 Claude provider의 `claude-sonnet-4-5` 모델로 실행한다.

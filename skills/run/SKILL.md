...
이 skill은 대상 프로젝트 루트 cwd에서 실행한다. `scripts/`와 `src/`는 대상 프로젝트가 아니라 built plugin/repo 안의 디렉토리다.
Claude Bash tool, zsh, bash, interactive shell 모두에서 `BASH_SOURCE[0]`로 skill 파일 위치를 추정하지 않는다.

```bash
# BUILT_PLUGIN_DIR는 설치된 built plugin/repo의 절대 경로다.
# 예: plugin 개발 checkout에서 실행 중이면 export BUILT_PLUGIN_DIR="$(git rev-parse --show-toplevel)"

...
**PoC 4개 (실측 필수)**:
- PoC-1: `/built:plan` 시작 후 interactive 세션이 feature worktree로 전환되는지
- PoC-2: stream-json 이벤트 + 공용 runtime(progress/state/logs) 갱신
- PoC-3: AskUserQuestion 다중 연속 호출 ★ Plan 구현 결정
- PoC-4: feature worktree 재진입 + `.worktreeinclude` + `claude -p` 실행 경로 일치

**Phase 1**:
- `/built:init`, `/built:plan`, `/built:do` (포그라운드)
- `$RUNTIME/runs/<name>/state.json`, `progress.json`, `.built/runs/<name>/do-result.md`

...
```
~/.claude/plugins/marketplaces/built-marketplace/
├── .claude-plugin/
│   ├── marketplace.json
│   └── plugin.json
├── skills/
...
│   ├── pipeline-runner.js             # #!/usr/bin/env node, Do/Check/Iter/Report 오케스트레이션
│   ├── progress-writer.js             # stream-json → progress.json 실시간 파이프
│   ├── result-to-markdown.js          # progress.json → do/check/report-result.md 포매팅
│   ├── update-index.js                # features-index.md 자동 생성
│   ├── frontmatter.js                 # YAML 최소 subset 파서
│   └── hooks-parser.js
└── servers/                           # MCP 서버 (선택)
    └── built-pdca-server/
        ├── index.js                   # JSON-RPC 2.0 over stdio, deps 0
...
| Skills + `disable-model-invocation` | deterministic entrypoint용 skill |
...
팀 `.claude/settings.json`에 마켓플레이스 등록만 하면 자동 배포:
...
플러그인이 설치되는 위치는 `~/.claude/plugins/marketplaces/built-marketplace/`이고, 프로젝트 레포에는 여전히 실행 스크립트를 추가하지 않습니다.

...
Claude Code 위에서 feature를 **"built 상태"까지 자동으로 끌고 가는** AI 개발 워크플로우 도구.
...
- **`/built:init`은 프로젝트 1회 bootstrap** — feature 시작 명령이 아님
- **`/built:plan <feature>`부터 feature worktree lifecycle 시작** — 원본 레포에서 시작하지만 실제 Plan/Run은 같은 feature worktree에서 진행
- **Plan 진입은 `EnterWorktree` 기준** — interactive 세션이 feature worktree 컨텍스트로 전환됨
- **live state는 worktree 밖 공용 runtime** — 문서/코드는 feature worktree 안, `state/progress/logs`는 공용 경로
...
| `/built:plan <feature>` | feature worktree 생성 또는 재사용 후 Plan+Design 대화식 인터뷰 |
| `/built:run <feature>` | 같은 feature worktree에서 Do → Check → Iter → Report 자동 실행 |
...
```
Project bootstrap
  └─ /built:init — 프로젝트 기본 구조 준비 (최초 1회)

Plan (사람, 원본 레포에서 시작)
  └─ /built:plan — feature worktree 생성/재사용 후 `EnterWorktree`로 그 컨텍스트에 진입
...
Automation (Claude, 같은 feature worktree)
  └─ /built:run
       ├─ Do      — claude --bare -p + stream-json → do-result.md
       ├─ Check   — claude --bare -p + --output-format json + --json-schema → check-result.md
...
claude --bare -p --output-format stream-json --verbose
  ...
  ├─ $RUNTIME/runs/<name>/logs/<phase>.jsonl  ← 원본 이벤트 로그
  ▼
progress-writer.js         ← 실시간 파이프, atomic write
  │
  ▼
$RUNTIME/runs/<name>/progress.json  ← 기계 전용 snapshot (session_id, 수정된 파일, 턴 수, 비용)
  │  result 이벤트 수신 시 1회 실행
  ▼
result-to-markdown.js      ← 템플릿 포매팅
  │
  ▼
.built/runs/<name>/do-result.md   ← 사람용 + Obsidian용 (frontmatter + 본문)
```

| 레이어 | 형식 | 용도 |
|---|---|---|
| `claude -p` 출력 | stream-json | 기계 프로토콜 |
| `$RUNTIME/logs/<phase>.jsonl` | JSON Lines | 원본 이벤트 로그 |
| `$RUNTIME/progress.json` | JSON | 실시간 진행 snapshot |
| `.built/runs/<name>/*.md` | Markdown + frontmatter (git) | 사람이 읽고 Obsidian이 시각화 |

**Check**는 프롬프트 + `--json-schema`로 구조화 응답을 강제하고 `structured_output`을 Markdown 템플릿으로 변환.
**Report**는 Claude가 이미 Markdown으로 응답 → frontmatter만 prepend.
**Plan**은 변환 레이어 없음 — 원본 레포에서 시작하지만 `EnterWorktree`로 feature worktree 컨텍스트에 진입한 interactive 세션이 직접 `.md` 작성.

### Phase 감지는 파일 존재로

| 조건 | 현재 Phase |
|---|---|
| feature worktree 없음 | not_started |
| feature worktree 있음 + `features/<name>.md` 없음 | plan |
| shared `state.json.current_phase == "do"` | do |
| shared `state.json.current_phase == "check"` | check |
| `check-result.md` frontmatter `status: needs_changes` | iter |
| `report.md` 존재 + shared `state.json.status == "completed"` | report_done |
...
| 기능 | 활용처 |
|---|---|
| `EnterWorktree` | interactive Plan 세션을 feature worktree 컨텍스트로 전환하고 필요시 새 worktree 생성 |
...
**공용 runtime 상태 (git 바깥)**:
- `$RUNTIME/locks/`
- `$RUNTIME/registry.json`
- `$RUNTIME/runs/<name>/state.json`, `progress.json`, `logs/`, `iterations/`
...
- built 자체 dry-run 모드 제공 (`/built:run --dry-run` 또는 config 기반)
...
- PoC-1~4: EnterWorktree 전환, stream-json, AskUserQuestion, worktree 재진입 실증
- `/built:init`, `/built:plan`, `/built:do` (포그라운드)
- `state.json`, `progress.json`, `do-result.md` 스키마 확정

### Week 2: 자동화 연결 ✅

- `pipeline-runner.js`, `progress-writer.js`, `result-to-markdown.js`, `frontmatter.js`
- `/built:check`, Iter 루프, `/built:run` (Do→Check→Iter→Report 전체 오케스트레이션)
- `report.md` 생성, Notification hook

### Week 3: 안정화 ✅

- `/built:status`, `/built:list`, `/built:resume`, `/built:abort`
- `/built:sanitize` + pre-commit hook
- `/built:validate`, `/built:hooks-inspect`

### Week 4+: 고도화 (진행 중)

- 멀티 피처 동시성 (registry.json + locks/)
- Pipeline hooks 실행 엔진 (before_do/after_do/after_check/after_report)
- Iter 수렴 감지 강화 (C4 위험 대응)
- `/built:plan` plan-draft.md 중간 저장 (C2 위험 대응)
- E2E 통합 테스트 시나리오
- 마켓플레이스 배포 구조
...
**구현 완료 (Week 1~3)**, Week 4+ 고도화 진행 중. 최종 업데이트: 2026-04-24.

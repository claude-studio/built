...
- **Run**: `/built:run`이 built plugin/repo의 `scripts/run.js`를 절대 경로로 호출해 파이프라인을 오케스트레이션
...
npm run check:plugin-release   # marketplace/plugin release package 검증
```
...
provider별 smoke 테스트와 release 전 검증 기준은 [`docs/smoke-testing.md`](docs/smoke-testing.md), [`docs/ops/plugin-release-checklist.md`](docs/ops/plugin-release-checklist.md)를 참조하세요.
...
| preset 스킬 | 기본 `/built:run` | `/built:run-codex` |
...
project root에서 `node scripts/sanitize.js`를 실행하면 `.built/runs/`와 `.built/features/`의 Markdown/JSON/JSONL public artifact를 함께 검사합니다. execution worktree에서는 그 worktree의 canonical `.built/features/<feature>/`만 대상으로 하며 다른 worktree나 project 밖 경로를 자동 탐색하지 않습니다.

...
## Start/PR Safety Gate

Builder는 코드 수정, 테스트 실행, PR 생성 직전에 같은 issue를 다시 확인한다.

- `multica issue get <issue-id> --output json`으로 `status=in_progress`와 assignee가 Builder인지
  확인한다.
- 최신 handoff/result/correction comment를 확인한다. "정정", "되돌림", "시작하면 안 됨",
  "중단" 같은 지시가 있으면 작업을 시작하거나 계속하지 않는다.
- 현재 실행 대상 issue가 Builder에게 assign되어 있지 않거나 `backlog`, `done`, `blocked`,
  `in_review`로 바뀌었으면 코드 변경, commit, push, PR 생성, status 변경을 하지 않는다.
  한글/KST 코멘트로 "현재 assignee/status가 달라 이 실행은 무시한다"는 근거만 남기고 종료한다.
- PR 생성 직전에는 같은 BUI 번호의 open PR과 head branch를 다시 조회한다. 기존 canonical PR이
  있으면 새 PR을 만들지 않고 기존 branch에 누적한다. canonical이 불명확하면 push/PR 생성 없이
  Coordinator 판단을 요청한다.
- 잘못 시작된 실행에서 이미 로컬 변경이 생겼다면 원격 push나 PR 없이 보존 상태를 코멘트에
  요약한다. 다른 실행의 정상 branch나 PR을 닫거나 되돌리지 않는다.

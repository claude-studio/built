...
10. Codex structured output 경로에서는 app-server에 전달되는 `outputSchema`가 JSON Schema object 자체인지 확인한다.
    `{ schema: ... }` wrapper를 전달하지 않고, 최상위 `type`이 `"object"`인지 테스트로 고정한다.
11. plan synthesis schema가 Codex strict schema 조건인 `additionalProperties: false`와 모든 properties의 `required` 포함을 만족하는지 확인한다.
12. Codex provider가 최종 JSON text를 `structuredOutput`으로 반환하고, plan synthesis normalization이 provider-neutral 결과를 소비하는지 확인한다.
13. 기본 검증은 `npm test`, `npm test -- --unit`, `node scripts/smoke-codex-plan-synthesis.js`의 skip 동작 확인으로 마무리한다.
14. 실제 provider 확인이 필요할 때만 `BUILT_CODEX_PLAN_SYNTHESIS_SMOKE=1 node scripts/smoke-codex-plan-synthesis.js` 또는 `npm run test:smoke:codex:plan`을 실행한다.

...
8. `scripts/hooks-inspect.js`가 실제 `HOOK_POINTS`와 같은 hook point를 보여주는지 확인한다.
9. 테스트는 최소 `test/hooks-runner.test.js`를 실행하고, provider event 계약 문서를 바꿨다면 관련 문서/KG 민감정보 점검도 함께 실행한다.
10. PR handoff에는 provider-aware context 필드, hook 실패 정책 변경, 민감정보 제외 범위, 기존 hook 호환 위험을 명시한다.

...
4. provider raw event는 표준 이벤트로 normalize하고, fake provider fixture로 산출물 필수 구조 동일성을 검증한다.
5. 기존 pipeline 테스트, 신규 provider 단위 테스트, fake provider E2E를 함께 실행한다.
6. 리뷰에서 산출물 위치, CLI 호출 방식, 진행 상태 기록이 외부 동작 불변인지 확인한다.
7. KG에는 issue record와 provider architecture decision을 분리해 남긴다.
...
- `phase_end` 또는 `error` 이후 이벤트를 허용하지 않는다. terminal 이후 이벤트는 progress/result 최종 상태를 뒤집을 수 있다.
- 실제 Codex 연결 전에는 `normalizeCodex` passthrough 가정을 명시하고, app-server notification shape가 확인되면 매핑 fixture를 갱신한다.

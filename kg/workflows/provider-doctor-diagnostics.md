...
type: workflow
date: 2026-04-26
validated_by: [BUI-168]
tags: [provider, diagnostics, codex, broker, smoke, regression]
---
...
1. `node scripts/provider-doctor.js` 또는 `npm run doctor`로 기본 환경 점검을 실행한다.
2. 자동화나 CI에서 결과를 소비해야 하면 `node scripts/provider-doctor.js --json`을 사용한다.
3. 특정 feature 설정까지 확인해야 하면 `--feature <featureId>`를 추가한다.
4. Codex 설치 점검은 binary 없음과 app-server 미지원 상태를 분리해 본다.
5. 인증 점검은 `codex login status` 기반 결과를 확인하고, 실패하면 사용자에게 `codex login` 조치를 안내한다.
6. broker session 점검은 PID 생존 여부를 먼저 보고, unix endpoint는 socket 파일 존재 여부까지만 확인한다.
7. broker lock 점검은 파싱 실패, stale PID, 30초 초과 lock을 stale 후보로 취급한다.
8. run-request provider 설정은 `parseProviderConfig()` 결과로 검증하고, 별도 doctor 전용 규칙을 만들지 않는다.
9. feature registry에 running feature가 있으면 동시 실행과 broker 경합 가능성을 확인한다.
10. doctor가 `fail`을 반환하면 smoke를 실행하기 전에 표시된 `action`을 먼저 처리한다.
11. doctor가 `warn`만 반환하면 실행은 가능하지만 stale broker, lock, registry 상태를 확인한 뒤 smoke 여부를 판단한다.
12. doctor가 `ok`여도 실제 provider 응답, 산출물, file contract 검증은 smoke 테스트에서 별도로 확인한다.

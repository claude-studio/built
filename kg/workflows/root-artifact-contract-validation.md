...
6. Report는 `root-context.json`의 `artifact_paths.kg_draft`와 report frontmatter의 `kg_draft`, `kg_draft_target_root`가 target project 기준 `kg/issues/<FEATURE>.md`를 가리키는지 확인한다.
7. plugin root 또는 helper `__dirname` 아래 `.built`나 `kg/issues`에 target artifact가 생기지 않는지 확인한다.
...
- Report KG draft는 target project `kg/issues` 기준이다. 설치형 plugin package/cache의 `kg/issues`에 draft를 쓰면 root drift로 본다.

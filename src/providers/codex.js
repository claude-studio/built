 ...
 * 2. phase_start 이벤트 emit
 * 3. codex app-server 프로세스 spawn → thread/start → turn/start
 * 4. notification을 표준 이벤트로 변환해 onEvent 콜백 전달
 * 5. turn/completed → phase_end 후 resolve
 * 6. timeout → turn/interrupt 시도 → error 이벤트 후 resolve
 *
 * @param {object} opts
 ...
 */
function runCodex({
  prompt, model, effort, sandbox, timeout_ms, outputSchema,
  ...
  return new Promise((resolve) => {
    ...
    const timeoutTimer = setTimeout(async () => {
      ...
      if (finalThreadId && finalTurnId) {
        try {
          await client.request('turn/interrupt', { threadId: finalThreadId, turnId: finalTurnId });
        } catch (_) {}
      }
